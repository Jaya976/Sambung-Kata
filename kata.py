...
def get_level(tc):
    if tc <= 20: return "🟢 Easy (2/3 Huruf)"
    elif tc <= 40: return "🟡 Medium (Putaran 40+)"
    elif tc <= 60: return "🔴 Hard (Putaran 60+)"
    else: return "🏆 Master"
...
async def peraturan_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = ("📖 <b>PERATURAN PERMAINAN</b>\n━━━━━━━━━━━━━━━\n"
            ...
            "2. 📊 <b>Minimal Huruf:</b> Level akan meningkat seiring putaran game.\n"
            "3. 🚫 <b>Larangan:</b>\n"
...
async def mulai_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.effective_chat.type == Chat.PRIVATE:
        ...
        SOLO_ROOMS[uid] = {'suffix': start_char, 'used_words': {}}
...
async def usir_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if room and room['active']:
        # Limit Usir 1x per game (Permintaan User)
...
async def ganti_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if room and room['active'] and uid == room['players'][room['turn']]:
        if room['ganti_limit'].get(uid, 0) >= 1: return
...
async def handle_all(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.effective_chat.type == Chat.PRIVATE and u.id in SOLO_ROOMS:
        ...
        if word in BANNED_NAMES or word not in dictionary or not word.startswith(solo['suffix']):
            update_points(u.id, u.first_name, -1); return await update.message.reply_text("❌ Salah! -1 Poin", parse_mode=ParseMode.HTML)
        solo['used_words'][word] = datetime.now() + timedelta(minutes=30)
        s_len = 3 if len(word) >= 5 else 2
        solo['suffix'] = word[-s_len:]; update_points(u.id, u.first_name, 1)
    ...
    update_points(u.id, u.first_name, 10); await next_turn_msg(context, cid)
